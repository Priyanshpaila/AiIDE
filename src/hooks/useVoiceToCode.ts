...
export function useVoiceToCode(
  ...
  const processAudio = async (audioBlob: Blob) => {
    try {
      // Convert browser audio (WebM/Ogg) to 16kHz 16-bit PCM WAV for Whisper
      ...
      // If we finish normally
      onCodeGenerated("", true);
      setStatus("idle");
    } catch (err) {
      console.error("AI processing error:", err);
      // Output the actual error to the editor so we can see what's failing
